...
// Plain RAM — no deep sleep persistence needed in this sketch
static int  fram_record_count = 0;
static char simulated_fram[MAX_RECORDS][MAX_PAYLOAD_SIZE];

void fram_init() {
    Serial.println("F-RAM HAL: Initialised (simulated in RAM).");
}
...
int fram_get_records(ProbeRecord_t* out_records, int max_records) {
    ...
    for (int i = 0; i < fram_record_count && count < max_records; i++) {
        ...
        if (parsed == 15) {
            out_records[count] = build_record(
                entry_num, time_ms, temp, pressure,
                spec[0], spec[1], spec[2], spec[3], spec[4],
                spec[5], spec[6], spec[7], spec[8], spec[9], spec[10]);
            count++;
